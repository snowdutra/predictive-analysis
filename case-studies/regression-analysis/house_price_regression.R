...
csv_path <- file.path(Sys.getenv('USERPROFILE'), 'Downloads', 'train.csv')  
if (!file.exists(csv_path)) stop('train.csv não encontrado em Downloads: ', csv_path)  

df <- read_csv(csv_path)

# selecionar colunas e remover NA
cols <- c('SalePrice','OverallQual','GrLivArea','GarageCars','TotalBsmtSF','YearBuilt','FullBath','LotArea')  
...
# modelo completo
formula_full <- as.formula('SalePrice ~ OverallQual + GrLivArea + GarageCars + TotalBsmtSF + YearBuilt + FullBath + LotArea')  
m_full <- lm(formula_full, data = df)

# busca exaustiva por AIC (todos os subconjuntos)
predictors <- c('OverallQual','GrLivArea','GarageCars','TotalBsmtSF','YearBuilt','FullBath','LotArea')  
best_aic <- Inf
best_model <- NULL
best_vars <- NULL
for (k in 1:length(predictors)){
  ...
  for (vars in combos){
    f <- as.formula(paste('SalePrice ~', paste(vars, collapse = '+')))  
    m <- lm(f, data = df)
    a <- AIC(m)
    if (a < best_aic){
      best_aic <- a
...
# preparar saída
out_file <- 'analysis_output_questao01.txt'  
sink(out_file)
cat('--- Modelo completo ---\n')  
...
cat(sprintf('R2: %.4f  R2 ajustado: %.4f\n\n', summary(best_model)$r.squared, summary(best_model)$adj.r.squared))  
