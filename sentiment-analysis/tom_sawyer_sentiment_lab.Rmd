...
# Carregar o texto
setwd("C:/Users/gustavo.telles/Documents/GitHub/Predictive-Analysis/tom-sentiment-lab")  
tom_raw <- readLines("C:/Users/gustavo.telles/Documents/GitHub/Predictive-Analysis/tom-sentiment-lab/tom_sawyer.txt", encoding = "UTF-8")  
tom_tbl <- tibble(
  text = tom_raw,  
...
tom_tbl <- tom_tbl %>%
  mutate(chapter = cumsum(str_detect(text, regex("CHAPTER\\s+[IVXLCDM0-9]+", ignore_case = TRUE))))  
```
...
sent_bing_chapter_resumo <- sent_bing_chapter %>% select(chapter, saldo) %>% rename(saldo_bing = saldo)  
...
comparativo_chapter <- sent_bing_chapter_resumo %>%
  left_join(sent_nrc_chapter %>% select(chapter, saldo_nrc), by = "chapter") %>%  
...
comparativo_chapter %>% head(10) %>% knitr::kable(caption = "Exemplo de tabela comparativa dos 10 primeiros capítulos")  
```
